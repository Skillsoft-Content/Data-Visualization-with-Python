...
sub = lane_df[lane_df["LANE"] == sel_lane]
with col_b: sel_prod = st.selectbox("Product",   ["All"]+sorted(sub["PRODUCT"].dropna().unique().tolist()))
with col_c: sel_rt   = st.selectbox("Rate type", ["All"]+sorted(sub["RATE_TYPE"].dropna().unique().tolist()))
if sel_prod != "All": sub = sub[sub["PRODUCT"]   == sel_prod]
if sel_rt   != "All": sub = sub[sub["RATE_TYPE"] == sel_rt]
if sub.empty: st.warning("No matching combination."); st.stop()
...
with sc1: price_change  = st.slider("Base price change (%)",   -30, 20, -5,  step=1) / 100
with sc2: disc_change   = st.slider("Additional discount (%)",   0, 25,  0,  step=1) / 100
with sc3: elasticity    = st.slider("Price elasticity", 0.5, 3.0, float(THRESHOLDS["price_elasticity"]), step=0.1)

sc4,sc5,sc6 = st.columns(3)
with sc4: max_sac = st.slider("Max GRATE sacrifice (%)", 5, 50, int(THRESHOLDS["max_grate_sacrifice_pct"]*100), step=5) / 100
with sc5: min_sl  = st.slider("Min shipment lift", 0.0, 20.0, float(THRESHOLDS["min_shipment_lift"]), step=0.5)
with sc6:
    total_pc = price_change + disc_change
    color = "green" if total_pc < 0 else "red"
    st.markdown(f"**Total price change**")
    st.markdown(f"<h2 style='color:{color}'>{total_pc:.0%}</h2>", unsafe_allow_html=True)

result = run_scenario(row.to_dict(), price_change_pct=price_change, discount_change_pct=disc_change,
                      max_grate_sacrifice_pct=max_sac, min_shipment_lift=min_sl, elasticity=elasticity)

st.divider()
st.subheader("Scenario Results")
rec = result["recommendation"]
badge = {"Good opportunity":"success","Too risky – negative GRATE lane":"error",
         "GRATE risk too high":"error","Not enough shipment lift":"warning",
         "Expected GRATE lift is negative":"warning","Insufficient data":"info",
         "Needs pricing review":"info"}.get(rec, "info")
getattr(st, badge)(f"**Recommendation: {rec}**")

rc1 = st.columns(4)
rc1[0].metric("Current Win Prob",   format_pct(result["current_win_prob"]))
rc1[1].metric("Scenario Win Prob",  format_pct(result["scenario_win_prob"]),
              delta=f"{result['scenario_win_prob']-result['current_win_prob']:+.1%}")
rc1[2].metric("Current Exp. Ships", f"{result['current_expected_shipments']:.1f}")
rc1[3].metric("Scenario Exp. Ships",f"{result['scenario_expected_shipments']:.1f}",
              delta=f"{result['shipment_lift']:+.1f}")

rc2 = st.columns(4)
rc2[0].metric("Current Exp. GRATE",  format_currency(result["current_expected_grate"]))
rc2[1].metric("Scenario Exp. GRATE", format_currency(result["scenario_expected_grate"]),
              delta=format_currency(result["grate_lift"]))
rc2[2].metric("GRATE Sacrifice/Ship",format_currency(result["grate_sacrifice_per_shipment"]))
rc2[3].metric("Expected GRATE Lift", format_currency(result["grate_lift"]))
...
for pc in price_range:
    r = run_scenario(row.to_dict(), price_change_pct=pc, discount_change_pct=0,
                     max_grate_sacrifice_pct=max_sac, min_shipment_lift=min_sl, elasticity=elasticity)
    sens_data.append({"Price Change": pc, "Expected GRATE": r["scenario_expected_grate"],
...
fig.add_vline(x=price_change, line_dash="dash", line_color="red", annotation_text="Current")
fig.update_layout(template="plotly_white", title="Expected GRATE vs Price Change",
                  xaxis_title="Price Change", yaxis_title="Expected GRATE ($)", xaxis_tickformat=".0%")
...
fig2.update_layout(template="plotly_white", title="Win Probability vs Price Change",
