...
def _get_lane_summary(df: pd.DataFrame) -> pd.DataFrame:
...
# Date range (filter on quote level, then recompute lane summary)
...
filt = filt[filt["ACTUAL_WIN_RATE"] <= max_wr]
filt = filt[filt["GRATE"]           >= min_grate]
...
display_cols = {
    ...
    "ACTUAL_WIN_RATE":             "Actual WR",
    "PREDICTED_WIN_RATE":          "Predicted WR",
    "REVENUE":                     "Revenue ($)",
    "GRATE":                       "GRATE ($)",
    "GRATE_PER_SHIPMENT":          "GRATE/Ship ($)",
    "GRATE_PER_KG":                "GRATE/KG ($)",
    "AVG_KG":                      "Avg KG",
    "AVG_PRICE_PER_KG":            "Price/KG ($)",
    "AVG_DISCOUNT_PCT":            "Avg Disc (%)",
    "EXPECTED_SHIPMENTS":          "Exp. Ships",
    "EXPECTED_GRATE":              "Exp. GRATE ($)",
    "SCENARIO_EXPECTED_SHIPMENTS": "Scen. Ships",
    "SCENARIO_EXPECTED_GRATE":     "Scen. GRATE ($)",
    "EXPECTED_SHIPMENT_LIFT":      "Ship. Lift",
    "EXPECTED_GRATE_LIFT":         "GRATE Lift ($)",
    "RECOMMENDATION":              "Recommendation",
    "CONFIDENCE_LEVEL":            "Confidence",
    "REASON":                      "Reason",
}

available = {k: v for k, v in display_cols.items() if k in filt.columns}
out = filt[list(available.keys())].rename(columns=available)

# Format numeric display
pct_cols  = ["Actual WR", "Predicted WR", "Avg Disc (%)"]
cur_cols  = ["Revenue ($)", "GRATE ($)", "GRATE/Ship ($)", "GRATE/KG ($)",
             "Price/KG ($)", "Exp. GRATE ($)", "Scen. GRATE ($)", "GRATE Lift ($)"]
num_cols  = ["Quotes", "Shipments", "Avg KG", "Exp. Ships", "Scen. Ships", "Ship. Lift"]
...
for c in num_cols:
    if c in out.columns:
        out[c] = out[c].apply(lambda x: f"{x:,.1f}" if pd.notna(x) else "N/A")

st.dataframe(out, use_container_width=True, height=600)

# ── Lane selection for drilldown ──────────────────────────────────────────────

st.divider()
lane_options = filt["LANE"].tolist() if "LANE" in filt.columns else []
if lane_options:
    selected_lane = st.selectbox("Select a lane to drill into:", [""] + lane_options)
    if selected_lane:
        st.session_state["selected_lane"] = selected_lane
        st.success(f"Lane **{selected_lane}** selected. Navigate to **Drilldown** to explore.")
