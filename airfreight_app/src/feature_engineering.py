...
Call run_feature_engineering(df, reference_df) where:
  - df           : the dataset to transform
  - reference_df : the dataset used to compute historical aggregates
                   (pass the training set here to avoid leakage)
"""
...
# ── Historical aggregates (computed on reference_df, applied to df) ───────────

def _safe_merge(df, stats, on):
    return df.merge(stats, on=on, how="left")


def add_historical_features(df: pd.DataFrame, reference_df: pd.DataFrame) -> pd.DataFrame:
    ...
    lane_stats = (
        ...
        .agg(
            LANE_WIN_RATE=("WON", "mean"),
            LANE_QUOTE_COUNT=("QUOTE_GROUP_ID", "nunique"),
            LANE_SHIPMENT_COUNT=("WON", "sum"),
            LANE_AVG_GRATE=("GRATE", "mean"),
            LANE_AVG_REVENUE=("REVENUE", "mean"),
            LANE_AVG_KG=("QUOTE_WGT_KG", "mean"),
        )
        .reset_index()
    )
    df = _safe_merge(df, lane_stats, "LANE")

    # Customer-level
    cust_stats = (
        reference_df.groupby("LASSO_ID")
        .agg(
            CUST_WIN_RATE=("WON", "mean"),
            CUST_QUOTE_COUNT=("QUOTE_GROUP_ID", "nunique"),
        )
        .reset_index()
    )
    df = _safe_merge(df, cust_stats, "LASSO_ID")

    # Customer × Lane
    cl_stats = (
        reference_df.groupby(["LASSO_ID", "LANE"])
        .agg(CUST_LANE_WIN_RATE=("WON", "mean"))
        .reset_index()
    )
    df = _safe_merge(df, cl_stats, ["LASSO_ID", "LANE"])

    # Rate type
    rt_stats = (
        reference_df.groupby("RATE_TYPE")
        .agg(RATE_TYPE_WIN_RATE=("WON", "mean"))
        .reset_index()
    )
    df = _safe_merge(df, rt_stats, "RATE_TYPE")

    # Product
    prod_stats = (
        reference_df.groupby("PRODUCT")
        .agg(PRODUCT_WIN_RATE=("WON", "mean"))
        .reset_index()
    )
    df = _safe_merge(df, prod_stats, "PRODUCT")

    # Fill missing with global win rate or 0
    for col in df.columns:
        if col.endswith("_WIN_RATE"):
            df[col] = df[col].fillna(global_wr)
        elif col.endswith("_COUNT") or col.endswith("_SHIPMENT_COUNT"):
...
def run_feature_engineering(df: pd.DataFrame, reference_df: pd.DataFrame = None) -> pd.DataFrame:
